...
const UpdateStore = ({ onSubmitSuccess }) => {
  ...
  const prevStoreCode = useRef(null);
  ...
  const handleStoreDelete = async () => {
    try {
      ...
      await axios.delete(`http://localhost:4000/api/deletestore/${storeCode}`);
     
      // Redirect to a different page or handle success as needed
      navigate("/dashboard/settings"); // Redirect to the home page, for example
    ...
    onSubmit: async (values, { setFieldError, resetForm }) => {
      try {
        ...
        const response = await axios.put(
          `http://localhost:4000/api/updatestore/${storeCode}`,
          values
        );
  ...
  const fetchData = useCallback(async () => {
    try {
      const response = await axios.get(
        `http://localhost:4000/api/store/${storeCode}`
      );
      ...
      const countryOption = Countries.find(
        (option) => option.label === storeData.country.label
      );
      ...
      prevStoreCode.current = storeCode; // Update after a successful call
  ...
  useEffect(() => {
    if (storeCode && storeCode !== prevStoreCode.current) {
      fetchData();
      prevStoreCode.current = storeCode;
    }
  }, [storeCode, fetchData]);

  // Remove fetchData and storeCode from the dependencies array
      ...
      <Box className="supplier-form-container">
        ...
        <Box className="supplier-form-field">
          ...
          <TextField
            ...
            error={
              formik.touched.storeCode && Boolean(formik.errors.storeCode)
            }
            helperText={
        ...
        <Box>
          ...
          >
            ...
            <DialogActions>
              ...
              <Button onClick={handleStoreDelete} autoFocus>
                Delete
              </Button>
            </DialogActions>
          </Dialog>
